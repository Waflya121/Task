...
**Важно:** это работает только при **локальном** запуске. Многие хостинги
(в т.ч. бесплатный план Render) блокируют исходящие SMTP-порты как защиту от
спама — при деплое туда письма отправляться не будут (см. раздел «Деплой» ниже).
...
Важно: SMTP (в т.ч. Gmail) работает только **локально**. Многие хостинги (в
том числе бесплатный план Render) блокируют исходящие SMTP-порты как защиту
от спама, и попытка подключения к SMTP-серверу будет падать по таймауту
независимо от правильности данных — это ограничение сети хостинга, не баг
кода. Если нужна реальная доставка писем с задеплоенного на Render backend —
единственный обходной путь — перейти на почтового провайдера с HTTP API
(Resend, SendGrid, Brevo и т.п.), у которого письма уходят обычным HTTPS-
запросом, а не через SMTP-порт.
...
5. В дашборде Render зайдите в **nova-server → Environment** и заполните:
   ...
   - `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `MAIL_FROM` — как в
     `server/.env.example`. **Учтите:** Render на бесплатном плане блокирует
     исходящие SMTP-порты, поэтому отправка писем с задеплоенного сервера
     работать не будет (регистрация/вход при этом всё равно работают — просто
     без реальной доставки письма). Если нужна рабочая отправка на Render —
     единственный вариант — перейти на почтового провайдера с HTTP API
     (Resend/SendGrid/Brevo), верифицировав там отправителя.
